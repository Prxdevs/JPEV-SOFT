...
SaleController.create = async (req, res) => {
  const {
    ...
    comment,
    hypothication,
    deliverynote_no,booking_ref,booking_date,mode_payment,vin_number,
    motor_number,charger_number,battery_number,colour,amt_words,bank_details,account_no,
    bank,branch,ifsc
  } = req.body;
  ...
  try {
    ...
    //INCLUDED GST-5% with cgst (2.5%) && sgst(2.5%)
    const preGstAmount = (parseInt(quantity) * parseInt(rate)) - parseInt(discount);
    const gstAmount = (5 / 100) * preGstAmount; // GST at 5%
    const amount = preGstAmount + gstAmount  + parseInt(shippingCost);
    ...
    const newSale = await new Sale({
      ...
      comment,
      hypothication,
      deliverynote_no,booking_ref,mode_payment,vin_number,motor_number,
      charger_number,battery_number,colour,booking_date,amt_words,bank_details,account_no,
      bank,branch,ifsc
    }).save();
